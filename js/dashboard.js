...
async function loadDashboard(){
  ...
  if(mob){mob.innerHTML=recent.length?recent.map(o=>`
    ...
    </div>`).join(''):'<div style="text-align:center;color:var(--text-dim);padding:2rem;font-size:0.85rem;">Sin ordenes aun</div>';}function renderPorCobrar(list){let wrap=document.getElementById('dash-por-cobrar');if(!wrap){wrap=document.createElement('div');wrap.id='dash-por-cobrar';const mob=document.getElementById('dash-recent-mobile');if(mob){const sec=mob.closest('[style*="background"],[class*="card"],section')||mob.parentElement;if(sec&&sec.parentElement)sec.parentElement.insertBefore(wrap,sec);else if(mob.parentElement)mob.parentElement.insertBefore(wrap,mob);}}if(!list||!list.length){wrap.innerHTML='';return;}const hoy=Date.now();const dias=f=>Math.floor((hoy-new Date(f).getTime())/(1000*60*60*24));const fmt=n=>'$'+Number(n).toLocaleString('es-MX',{minimumFractionDigits:2});wrap.innerHTML=`<div style="background:var(--bg2);border:1px solid var(--border);border-radius:14px;padding:1.1rem 1.25rem;margin-bottom:1.25rem;"><div style="font-family:var(--mono);font-size:0.7rem;color:var(--text-dim);text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.75rem;">\u{1F4B0} Por cobrar — ${list.length} lista${list.length!==1?'s':''}</div>${list.map(o=>{const d=dias(o.fecha_ingreso);const urg=d>7?'var(--red)':d>3?'var(--yellow)':'var(--green)';return`<div style="display:flex;justify-content:space-between;align-items:center;padding:0.6rem 0.9rem;border:1px solid var(--border);border-left:3px solid ${urg};border-radius:8px;margin-bottom:0.4rem;cursor:pointer;" onclick="verDetalle('${o.id}')"><div style="flex:1;min-width:0;"><span style="font-family:var(--mono);color:var(--blue);font-size:0.78rem;font-weight:700;">#${o.folio}</span> <span style="font-weight:600;font-size:0.85rem;">${o.clientes?.nombre||'?'}</span><div style="font-size:0.75rem;color:var(--text-dim);">${o.marca||''} ${o.modelo?'· '+o.modelo:''}</div></div><div style="text-align:right;flex-shrink:0;"><div style="font-family:var(--mono);font-size:0.75rem;color:${urg};font-weight:700;">${d===0?'Hoy':d===1?'1 día':d+' días'} esperando</div>${o.costo_estimado?`<div style="font-size:0.72rem;color:var(--text-dim);">${fmt(o.costo_estimado)}</div>`:''}</div></div>`;}).join('')}</div>`;}// ── MI PANEL & TAREAS ────────────────────────────────────(allOrdenes.filter(o=>o.estado==='listo'));
}

function renderPorCobrar(list){let wrap=document.getElementById('dash-por-cobrar');if(!wrap){wrap=document.createElement('div');wrap.id='dash-por-cobrar';const mob=document.getElementById('dash-recent-mobile');if(mob){const sec=mob.closest('[style*="background"],[class*="card"],section')||mob.parentElement;if(sec&&sec.parentElement)sec.parentElement.insertBefore(wrap,sec);else if(mob.parentElement)mob.parentElement.insertBefore(wrap,mob);}}if(!list||!list.length){wrap.innerHTML='';return;}const hoy=Date.now();const dias=f=>Math.floor((hoy-new Date(f).getTime())/(1000*60*60*24));const fmt=n=>'$'+Number(n).toLocaleString('es-MX',{minimumFractionDigits:2});wrap.innerHTML=`<div style="background:var(--bg2);border:1px solid var(--border);border-radius:14px;padding:1.1rem 1.25rem;margin-bottom:1.25rem;"><div style="font-family:var(--mono);font-size:0.7rem;color:var(--text-dim);text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.75rem;">\u{1F4B0} Por cobrar — ${list.length} lista${list.length!==1?'s':''}</div>${list.map(o=>{const d=dias(o.fecha_ingreso);const urg=d>7?'var(--red)':d>3?'var(--yellow)':'var(--green)';return`<div style="display:flex;justify-content:space-between;align-items:center;padding:0.6rem 0.9rem;border:1px solid var(--border);border-left:3px solid ${urg};border-radius:8px;margin-bottom:0.4rem;cursor:pointer;" onclick="verDetalle('${o.id}')"><div style="flex:1;min-width:0;"><span style="font-family:var(--mono);color:var(--blue);font-size:0.78rem;font-weight:700;">#${o.folio}</span> <span style="font-weight:600;font-size:0.85rem;">${o.clientes?.nombre||'?'}</span><div style="font-size:0.75rem;color:var(--text-dim);">${o.marca||''} ${o.modelo?'· '+o.modelo:''}</div></div><div style="text-align:right;flex-shrink:0;"><div style="font-family:var(--mono);font-size:0.75rem;color:${urg};font-weight:700;">${d===0?'Hoy':d===1?'1 día':d+' días'} esperando</div>${o.costo_estimado?`<div style="font-size:0.72rem;color:var(--text-dim);">${fmt(o.costo_estimado)}</div>`:''}</div></div>`;}).join('')}</div>`;}// ── MI PANEL & TAREAS ────────────────────────────────────────
